...
class InteractionAnalysisMessage(BaseModel):
    """交互分析消息"""
...
class MidsceneGenerationMessage(BaseModel):
    """Midscene生成消息"""
...
@type_subscription(topic_type="ui_analysis")
class UIAnalysisAgent(RoutedAgent):
    """UI元素识别专家智能体"""

    def __init__(self, description: str):
        super().__init__(description=description)
        logger.info(f"🔍 [UI分析智能体] 初始化: {description}")
...
- **结构化**: 严格遵循JSON格式，便于后续处理"""
    ...
    @message_handler
    async def handle_ui_analysis(
        self, message: UIAnalysisMessage, ctx: MessageContext
    ) -> None:
        """处理UI分析请求"""
...
请严格按照上述要求分析图片中的所有UI元素，并以JSON格式输出结果。"""
            ...
            # 使用AI核心框架创建智能体
            ui_agent = await create_assistant_agent(
                name="ui_analyst",
...
@type_subscription(topic_type="interaction_analysis")
class InteractionAnalysisAgent(RoutedAgent):
    """用户交互流程分析师智能体"""

    def __init__(self, description: str):
        super().__init__(description=description)
        logger.info(f"🔄 [交互分析智能体] 初始化: {description}")
...
```"""
    ...
    @message_handler
    async def handle_interaction_analysis(
        self, message: InteractionAnalysisMessage, ctx: MessageContext
    ) -> None:
        """处理交互分析请求"""
        start_time = time.time()
        conversation_id = message.conversation_id
        logger.info(f"🔄 [交互分析智能体] 开始分析 | 对话ID: {conversation_id}")
        logger.info(f"📝 用户需求: {message.user_requirement[:100]}...")
        ...
        try:
            ...
            # 使用AI核心框架创建智能体
            interaction_agent = await create_assistant_agent(
...
请根据UI元素，设计完整的用户交互流程，并严格按照JSON格式输出。"""
            ...
            logger.success(f"✅ 交互分析完成 - 总块数: {chunk_count}")

            # 保存到内存
            await save_to_memory(
                conversation_id,
                {
                    "type": "interaction_analysis_result",
            ...
            processing_time = time.time() - start_time
            logger.success(
                f"🎉 交互分析完成 | 对话ID: {conversation_id} | 耗时: {processing_time:.2f}秒"
            )

        except Exception as e:
            processing_time = time.time() - start_time
            logger.error(
                f"❌ 交互分析失败 | 对话ID: {conversation_id} | 耗时: {processing_time:.2f}秒"
            )
            logger.error(f"🔥 错误详情: {str(e)}")
            logger.error(f"📍 错误堆栈: {traceback.format_exc()}")
...
@type_subscription(topic_type="midscene_generation")
class MidsceneGenerationAgent(RoutedAgent):
    """Midscene.js自动化测试专家智能体"""

    def __init__(self, description: str):
        super().__init__(description=description)
        logger.info(f"🎯 [Midscene生成智能体] 初始化: {description}")
...
9. **多语言支持**: 支持中英文混合的界面描述"""
    ...
    @message_handler
    async def handle_midscene_generation(
        self, message: MidsceneGenerationMessage, ctx: MessageContext
    ) -> None:
        """处理Midscene生成请求"""
        start_time = time.time()
        conversation_id = message.conversation_id
        logger.info(f"🎯 [Midscene生成智能体] 开始生成 | 对话ID: {conversation_id}")
        logger.info(f"📝 用户需求: {message.user_requirement[:100]}...")
        ...
        try:
            ...
            # 使用AI核心框架创建智能体
            midscene_agent = await create_assistant_agent(
                name="midscene_generator",
                system_message=self.get_system_prompt(),
                conversation_id=conversation_id,
                auto_memory=True,
                auto_context=True,
...
请整合上述分析结果，设计完整的MidScene.js测试场景，严格按照JSON格式输出。"""
            ...
            logger.success(f"✅ Midscene生成完成 - 总块数: {chunk_count}")

            # 保存到内存
            await save_to_memory(
                conversation_id,
                {
                    "type": "midscene_generation_result",
            ...
            # 发送给脚本生成智能体
            script_message = ScriptGenerationMessage(
                conversation_id=conversation_id,
            ...
            await self.publish_message(
                ...
                topic_id=TopicId(type="script_generation", source=self.id.key),
            )

            processing_time = time.time() - start_time
            logger.success(
                f"🎉 Midscene生成完成 | 对话ID: {conversation_id} | 耗时: {processing_time:.2f}秒"
            )

        except Exception as e:
            processing_time = time.time() - start_time
            logger.error(
                f"❌ Midscene生成失败 | 对话ID: {conversation_id} | 耗时: {processing_time:.2f}秒"
            )
            logger.error(f"🔥 错误详情: {str(e)}")
            logger.error(f"📍 错误堆栈: {traceback.format_exc()}")
...
@type_subscription(topic_type="script_generation")
class ScriptGenerationAgent(RoutedAgent):
    """专业的测试脚本生成专家智能体"""

    def __init__(self, description: str):
        super().__init__(description=description)
        logger.info(f"📜 [脚本生成智能体] 初始化: {description}")

    def get_system_prompt(self) -> str:
        """获取系统提示词"""
        return """你是专业的测试脚本生成专家，专门将Midscene JSON测试用例转换为可执行的YAML脚本和Playwright脚本。
...
- **完整性**: 包含所有必要的步骤和验证"""
    ...
    async def handle_script_generation(
        self, message: ScriptGenerationMessage, ctx: MessageContext
    ) -> None:
        """处理脚本生成请求"""
        start_time = time.time()
        conversation_id = message.conversation_id
        logger.info(f"📜 [脚本生成智能体] 开始生成 | 对话ID: {conversation_id}")
        ...
        try:
            ...
            # 使用AI核心框架创建智能体
            script_agent = await create_assistant_agent(
...
请严格按照JSON格式输出两种脚本。"""
            ...
            logger.success(f"✅ 脚本生成完成 - 总块数: {chunk_count}")

            # 保存到内存
            await save_to_memory(
                conversation_id,
                {
                    "type": "script_generation_result",
            ...
            processing_time = time.time() - start_time
            logger.success(
                f"🎉 脚本生成完成 | 对话ID: {conversation_id} | 耗时: {processing_time:.2f}秒"
            )

        except Exception as e:
            processing_time = time.time() - start_time
            logger.error(
                f"❌ 脚本生成失败 | 对话ID: {conversation_id} | 耗时: {processing_time:.2f}秒"
            )
            logger.error(f"🔥 错误详情: {str(e)}")
            logger.error(f"📍 错误堆栈: {traceback.format_exc()}")
