...
Agent Platform 是一套构建在 **LangGraph / LangChain** 之上的通用 AI 智能体平台工作区，但它不是一个只适合 demo 的单体示例仓库。

这套框架的核心目标，是把 **企业级平台治理能力** 和 **智能体执行能力** 明确拆开：既能支持平台侧的安全、项目管理、审计、catalog 同步与治理，也能让 agent 侧专注图编排、模型装配、tools / MCP / skills 接入和快速迭代。
...
拆成四个应用，不是为了把仓库切碎，而是为了把职责边界和团队协作边界拉清楚：
